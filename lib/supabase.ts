...
function normalizeQuizExplanationLanguage(lang: string | null | undefined): 'ko' | 'jp' | '' {
  if (!((lang ?? '').trim())) return '';
  const key = normalizeWordContentLanguageKey(lang);
  if (key === 'ja') return 'jp';
  if (key === 'ko') return 'ko';
  return '';
}

function buildQuizExplanationMap(
  explanations: QuizExplanationRow[],
  learningLanguage: string
): Map<string, string> {
  const targetLanguage = normalizeQuizExplanationLanguage(learningLanguage) || 'ko';
  const explanationByQuestion = new Map<string, string>();

  for (const e of explanations) {
    if (!e.question_id) continue;
    const rowLanguage = normalizeQuizExplanationLanguage(e.language);
    if (rowLanguage === targetLanguage && !explanationByQuestion.has(e.question_id)) {
      explanationByQuestion.set(e.question_id, e.explanation ?? '');
    }
  }

  return explanationByQuestion;
}
...
export async function fetchQuizByDay(
  dayNum: number,
  learningLanguage: string = 'ko'
): Promise<DayQuizQuestion[]> {
  ...
  const explanationByQuestion = buildQuizExplanationMap(explanations, learningLanguage);
...
export async function fetchQuizByWeek(
  weekNum: number,
  learningLanguage: string = 'ko'
): Promise<FetchQuizByWeekResult> {
  const { data: quizData, error: quizError } = await supabase
    .from('quizzes')
